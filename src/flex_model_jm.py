...
class JointModel(nn.Module):
    """Wrapper combining a regression head and a classification head.

    The regressor is applied first to produce regression parameters (and optional
    latent features). These are concatenated with the original inputs and passed
    to the classifier.
    """
    ...
    def forward(self, hits, batch_name=None, flex_padding_mask=None, seq_lengths=None):
        ...
        params_pred, latent_feats = self.regressor(hits, batch_name, flex_padding_mask)
        parts = [hits, params_pred.detach()]
        if latent_feats is not None:
            parts.append(latent_feats)
        feats = torch.cat(parts, dim=-1)

        logits = self.classifier(feats, batch_name, flex_padding_mask)
        return logits, params_pred, padding_mask     
...
class TransformerRegressor(nn.Module):
    ...
    def __init__(self, inputfeature_dim: int, num_params: int, num_heads: int, 
                 embed_dim: int, num_layers: int, dim_feedforward: int, latent_dim: int = 0, use_flash_attention: bool=False,
                 is_causal: bool=False, dropout:float=0.0):
        ...
        self.reg_head = nn.Linear(embed_dim, num_params)
        self.latent_head = None  # set below if latent_dim > 0
        self.supervised_dim = num_params
    ...
    def forward(self, input, batch_name, flex_padding_mask):
        ...
        params_pred = self.reg_head(memory)
        latent_feats = self.latent_head(memory) if self.latent_head is not None else None
        

        return params_pred, latent_feats
...
def load_encreg(encreg_model_path, device, **model_kwargs):
    """Load a TransformerRegressor checkpoint and return it on device."""
    net = TransformerRegressor(**model_kwargs)
    model = torch.load(encreg_model_path, map_location=device)
